...
// Stateless status proxy — queries Poseidon directly.
...
// Stateless Utmify dispatch — frontend calls this on Pix create AND on approval.
app.post("/api/utmify/notify", async (req, res) => {
  if (!UTMIFY_API_TOKEN) {
...
app.post("/api/pixel/event", async (req, res) => {
  ...
  try {
    ...
    console.log("[Meta CAPI] Purchase response:", JSON.stringify(result));
    res.json(result);
  } catch (error) {
