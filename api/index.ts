...
function clientIpFrom(req: Request): string {
  ...
  let ip = (Array.isArray(xff) ? xff[0] : xff || req.ip || (req.socket as any)?.remoteAddress || "").toString();
  ip = ip.split(",")[0].trim();
...
router.get("/pix/status/:identifier", async (req: Request, res: Response) => {
...
router.post("/utmify/notify", async (req: Request, res: Response) => {
  if (!UTMIFY_API_TOKEN) {
...
router.post("/pixel/event", async (req: Request, res: Response) => {
...
// Mount router at both `/api` (when Vercel keeps the prefix) and `/` (after rewrite strips it).
app.use("/api", router);
app.use("/", router);
